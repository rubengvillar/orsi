...
export default function AccessoriesManager() {
    ...
    const [formData, setFormData] = useState<Partial<AluminumAccessory>>({
    ...
    async function fetchItems() {
        setLoading(true);
        const { data } = await supabase.from('aluminum_accessories').select('*').order('code');
        if (data) setItems(data);
        setLoading(false);
    }

    function handleOpenModal(item?: AluminumAccessory) {
        if (item) {
    ...
    async function handleDelete(item: AluminumAccessory) {
        if (!confirm(`¿Eliminar ${item.code}?`)) return;
        await supabase.from('aluminum_accessories').delete().eq('id', item.id);
        fetchItems();
    }

    async function handleUpdateStock(item: AluminumAccessory, newQty: number) {
        setLoading(true);
        const { error } = await supabase
            .from('aluminum_accessories')
    ...
    return (
        <div className="space-y-6">
            ...
            <Modal
                ...
                title={editingId ? 'Editar Accesorio' : 'Nuevo Accesorio'}
            >
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
                        />
                        <p className="text-xs text-slate-500 mt-1">Se mostrará una alerta si la cantidad es menor o igual a este valor.</p>
                    </div>
